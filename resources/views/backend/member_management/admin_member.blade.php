                            ...
                            <div class="card">
                                <div class="card-header border-transparent">
                                    ...
                                    <div class="card-tools">
                                        <a href="{{ route('admin.membersData.create') }}" class="btn btn-sm btn-info float-left" id="createMember">Add New
                                            Member</a>
                                ...
                                <div class="card-body p-0">
                                    <div class="table-responsive">
                                        <table id="contact_table" class="table m-0 member-table table-bordered" style="width:100%">
                                            <thead>
                                                <tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content bg-secondary">
                ...
                <form id="paymentMemberForm" method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer justify-content-between">
                        <button type="button" class="btn btn-outline-light" data-dismiss="modal">Close</button>
                        <button type="button" class="btn btn-outline-light" id="individualPaymentSubmitBtn">Submit</button>
                    </div>
                </form>
            </div>
            <!-- /.modal-content -->
...
$(document).ready(function() {
    $(function() {
        var table = $('.member-table').DataTable({
            ...
            searchable: true,
            "ajax": {
                "url": "{{ route('admin.membersData.index') }}",
                "data": function(data) {
                    //filter options
...
$(document).on('click', '#submitBtn', function () {
    ...
    $.ajax({
        type: 'post',
        url: "{{ route('admin.membersData.store') }}",
        data: formData,
        contentType: false,
        processData: false,
        success: function (response) {
...
$(document).on('change', '#approveRole', function (e) {
    e.preventDefault();
    var role = $(this).val();
    var roleid = $(this).closest('tr').find('.designationId').val();
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: 'get',
                url: "{{ route('admin.role.approve') }}", // Corrected syntax with quotes
                data: {
                    role: role,
                    roleid: roleid,
                },
...
$(document).on('blur','#paid', function(e){
    ...
    var due = total - paid
    $('#due').val(due)
    // alert(paid + ' ' +total + ' '+ due)
    $("#paid").attr("max", total);

    // // Optionally, you can update the "Paid Balance" input value if it exceeds the new max value
    // var paidBalance = parseFloat($("#paid").val());
    // if (paidBalance > totalBalance) {
    // $("#paid").val(totalBalance);
    // }
})
